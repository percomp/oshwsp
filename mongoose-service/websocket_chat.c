...
struct mg_connection_raspi = NULL;
...
static void handle_websocket_message(struct mg_connection *conn) {
  ...
  if (conn->content_len > 5 && !memcmp(conn->content, "init ", 5)) {
    // Raspberry pide un init y nos manda su identidad
...
static int ev_handler(struct mg_connection *conn, enum mg_event ev) {
  switch (ev) {
    case MG_REQUEST:
      if (conn->is_websocket) {
        ...
        return MG_TRUE;
          ...
          if (!strcmp(conn->uri, "/raspi")) {
             mg_send_file(conn, "raspi.html", NULL);  // Return MG_MORE after!
             return MG_MORE;
          } else if (!strcmp(conn->uri, "/index")) {
             caratula(conn, "INDEX!!!");
             return MG_TRUE;
          } else if (!strcmp(conn->uri, "/start")) {
             caratula(conn, "START!!!");
             return MB_TRUE;
          } else if (!strcmp(conn->uri, "/stop")) {
             caratula(conn, "STOP!!!!");
             return MG_TRUE;
          }
      }
    case MG_WS_CONNECT:
      ...
      mg_websocket_printf(conn, WEBSOCKET_OPCODE_TEXT, "%s", "init_ok");
      return MG_FALSE;
    case MG_CLOSE:
